...
char *movi_small(enum reg dest, long long imm){
        ...
        char *instr = malloc(17 * sizeof(char));
        strcpy(instr, "mov ");
        strcat(instr, reg_to_string(dest));
        strcat(instr, ", #");
        char imm_str[7];
...
char *movi(enum reg dest, long long imm){
        ...
        char *lsl = malloc(18 * sizeof(char));
        strcpy(lsl, "lsl ");
        ...
        char *addi = malloc(22 * sizeof(char));
        char imm_str[7];
...
char *push(enum reg reg){
        char *instr = malloc(21 * sizeof(char));
        strcpy(instr, "str ");
...
char *push_pair(enum reg reg1, enum reg reg2){
        char *instr = malloc(26 * sizeof(char));
        strcpy(instr, "stp ");
...
char *pop(enum reg reg){
        char *instr = malloc(19 * sizeof(char));
        strcpy(instr, "ldr ");
        strcat(instr, reg_to_string(reg));
        strcat(instr, ", [sp], #16");
        return instr;  
...
char *pop_pair(enum reg reg1, enum reg reg2){
        char *instr = malloc(24 * sizeof(char));
        strcpy(instr, "ldp ");
        strcat(instr, reg_to_string(reg1));
        strcat(instr, ", ");
        strcat(instr, reg_to_string(reg2));
...
char *add(enum reg dest, enum reg src1, enum reg src2){
        char *instr = malloc(18 * sizeof(char));
        strcpy(instr, "add ");
        strcat(instr, reg_to_string(dest));
        strcat(instr, ", ");
...
char *cmpi(enum reg reg, long long imm){
        assert((-65537 <= imm) && (imm <= 65535));
        char *instr = malloc(16 * sizeof(char));
        strcpy(instr, "cmp ");
        strcat(instr, reg_to_string(reg));
        strcat(instr, ", #");
        char imm_str[7];
...
char *ldr(enum reg dest, enum reg addr){
        char *instr = malloc(15 * sizeof(char));
        strcpy(instr, "ldr ");
        strcat(instr, reg_to_string(dest));
        strcat(instr, ", [");
...
char *memory_copy(enum reg src, enum reg dest, long long bytes){
        ...
        char *store_lr = malloc(20 * sizeof(char));
        strcpy(store_lr, "str lr, [sp, #-16]!");
        char *bl = malloc(10 * sizeof(char));
        strcpy(bl, "bl memcpy");
        char *restore_lr = malloc(18 * sizeof(char));
...
char *ret(){
        char *instr = malloc(4 * sizeof(char));
        strcpy(instr, "ret");
        return instr;
}

char *comment(char *text){
    char *val = malloc((strlen(text) + 4) * sizeof(char));
    strcpy(val, "// ");
    strcat(val, text);
    return val;
}
...
char *literal(char *text){
        char *val = malloc((strlen(text) + 1) * sizeof(char));
        strcpy(val, text);
        return val;
}

char *include(char *module){
        char *instr = malloc((strlen(module) + 14) * sizeof(char));
