...
bool evaluate_immediate(const struct parse_tree *tree, struct codegen_params *params, int64_t *result){
        ...
        if (tree->children->len == 1){
                return evaluate_immediate(tree->children->head->data, params, result);
        }
        ...
        switch (tree->children->head->data->data.type){
                case SYMBOL_LPAREN:
                        return evaluate_immediate(tree->children->head->next->data, params, result);
                case SYMBOL_UNSAFE:
                case SYMBOL_SAFE:
                        const struct parse_tree *expr_tree; load_child_at(expr_tree, tree, 3);
                        return evaluate_immediate(expr_tree, params, result);
                case SYMBOL_ASM:
                        return false;
        }
        ...
        if (!evaluate_immediate(op1_tree, params, &op1) || !evaluate_immediate(op2_tree, params, &op2)){
                return false;
        }
...
char *generate_binaryexpr(const struct parse_tree *tree, struct codegen_params *params){
        int64_t result;
        if (evaluate_immediate(tree, params, &result)){
...
char *generate_constant(const struct parse_tree *tree, struct codegen_params *params){
        long long imm = strtoll(tree->data.value, NULL, 10);
        return movi(REG_RESULT, imm);
}

char *generate_identifier(const struct parse_tree *tree, struct codegen_params *params){
        char *address = generate_address(tree, params);
        return concat(3, 
                address, 
                mov(REG_SCRATCH, REG_RESULT),
...
char *generate_from_tree(const struct parse_tree *tree, struct codegen_params *params){
        ...
        switch (symbol) {
                ...
                case SYMBOL_CONSTANT:
                        return generate_constant(tree, params);
                case SYMBOL_IDENTIFIER:
                        return generate_identifier(tree, params);
                case SYMBOL_TRUE:
